...
class AttentionBase(nn.Module):
    ...
    def forward(
        self,
        keys: FloatTensor,    # 形如(32, 1, 2048)
        values: FloatTensor,  # 形如(32, 1, 2048)
        queries: FloatTensor, # 形如(32, 1, 2048)
        attention_mask: BoolTensor   # 形如(32, 1, 1, 64)
    ) -> FloatTensor:
        keys = keys.reshape(keys.shape[:2] + (self.head_count, -1))  # 形如(32, 1, 32, 64)
...
class DalleBartEncoder(nn.Module):
    ...
    def forward(self, text_tokens: LongTensor) -> FloatTensor:
        attention_mask = text_tokens.not_equal(1)[:, None, None, :]   # shape = (2, 1, 1, text_token_count)   （2, 1, 1, 64）
        encoder_state = (
            self.embed_tokens.forward(text_tokens) +
            self.embed_positions.forward(self.pose_tokens)
        )    # encoder_state  形如(2, 64, 2048)   2048是embed_count
        ...
        return encoder_state  # 形如(2, 64, 2048)