...
class MinDalle:
    ...
    def generate_raw_image_stream(
        ...
        tokens = self.tokenizer.tokenize(text, is_verbose=is_verbose)   # 首个和最后一个token分别为开始token和结束token，中间是文本token
        ...
        with torch.cuda.amp.autocast(dtype=self.dtype):
            encoder_state = self.encoder.forward(text_tokens)   # 形如（2，64，2048）
        ...
        with torch.cuda.amp.autocast(dtype=self.dtype):
            ...
            text_tokens = text_tokens[expanded_indices]  # 形如（32，64）
            encoder_state = encoder_state[expanded_indices]  # 形如（32，64，2048）
            attention_mask = text_tokens.not_equal(1)[:, None, None, :]  # 形如（32，1，1，64）
            ...
            with torch.cuda.amp.autocast(dtype=torch.float32):
                if ((i + 1) % 32 == 0 and progressive_outputs) or i + 1 == 256:
                    yield self.image_grid_from_tokens(
                        image_tokens=image_tokens[:, 1:],  # image_tokens中索引0上的token是什么？？？
    ...
    def generate_image_stream(self, *args, **kwargs) -> Iterator[Image.Image]:
        ...
        for image in image_stream:
            image = image.to(torch.uint8).to('cpu').numpy()   # 这里从image_stream中取出的image就是VQGanDetokenizer类forward方法输出的形如(1024, 1024, 3)的tensor
